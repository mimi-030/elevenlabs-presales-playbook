...
| # | Scenario | The problem | The fix | The result |
|---|----------|-------------|---------|------------|
| [01](01-tts-cache/) | Publisher article narration | Audio regenerated on every reader visit; the bill multiplies | Generate at publish time, cache on a content fingerprint | Generate once, serve a million reads |
| [02](02-stream-safeclient/) | Latency and rate limits | 4 seconds of silence before the first word; 429s at peak | Stream sentence by sentence; concurrency gate plus jittered backoff | TTFB 3.6s to 0.9s, no user-facing errors |
